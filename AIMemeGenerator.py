...
parser.add_argument(
    "--userprompt",
    help="A meme subject or concept to send to the chat bot. If not specified, the user will be prompted to enter a subject or concept.",
)
parser.add_argument(
    "--memecount",
    help="The number of memes to create. If using arguments and not specified, the default is 1.",
)
parser.add_argument(
    "--imageplatform",
    help="The image platform to use. If using arguments and not specified, the default is 'clipdrop'. Possible options: 'openai', 'stability', 'clipdrop'",
)
parser.add_argument(
    "--temperature",
    help="The temperature to use for the chat bot. If using arguments and not specified, the default is 1.0",
)
parser.add_argument(
    "--basicinstructions",
...
parser.add_argument(
    "--nouserinput",
    action="store_true",
    help="Will prevent any user input prompts, and will instead use default values or other arguments.",
)
parser.add_argument(
    "--nofilesave",
    action="store_true",
    help="If specified, the meme will not be saved to a file, and only returned as virtual file part of memeResultsDictsList.",
)
args = parser.parse_args()

# Create a namedtuple classes
ApiKeysTupleClass = namedtuple(
    "ApiKeysTupleClass", ["openai_key", "clipdrop_key", "stability_key"]
)
...
def get_config(config_file_path):
    ...
    config_raw.optionxform = (
        lambda option: option
    )  # This must be included otherwise the config file will be read in all lowercase
    ...
    for section in config_raw.sections():
        for key in config_raw[section]:
            ...
            config[
                key
            ] = settingValue  # Do not use parseConfigSetting() here or else it will convert all values to lowercase
...
def get_api_keys(api_key_filename="api_keys.ini", args=None):
    ...
    try:
        ...
        )  # Could not read from config file, will try command-line arguments next

    # Checks if any arguments are not None, and uses those values if so
    if any(value is not None for value in vars(args).values()):
        openai_key = args.openaikey or openai_key
        clipdrop_key = args.clipdropkey or clipdrop_key
        stability_key = args.stabilitykey or stability_key
...
def write_log_file(
    userPrompt, AiMemeDict, filePath, logFolder, basic, special, platform
):
...
def check_for_update(
    currentVersion=version, updateReleaseChannel=None, silentCheck=False
):
    ...
    try:
        ...
        if response.status_code != 200:
            if response.status_code == 403:
                if silentCheck == False:
                    print(
                        print(
                            "\nError [U-4]: Got a 403 error (ratelimit reached) when attempting to check for update."
                        )
                    )
                    print(
                        print(
                            "This means you have been rate limited by github.com. Please try again in a while.\n"
                        )
                    )
                else:
                    print(
                ...
                print(
                    f"Error [U-3]: Got non 200 status code (got: {response.status_code}) when attempting to check for update.\n"
                )
                print(
        ...
        if "WinError 10013" in str(ox):
            print(
                f"WinError 10013: The OS blocked the connection to GitHub. Check your firewall settings.\n"
            )
        else:
        ...
        if silentCheck == False:
            print(str(e) + "\n")
            print(
                f"Error [Code U-1]: Problem while checking for updates. See above error for more details.\n"
            )
            print(
                "If this keeps happening, you may want to report the issue here: https://github.com/ThioJoe/Full-Stack-AI-Meme-Generator/issues"
            )
            ...
            print(
                f"Error [Code U-1]: Unknown problem while checking for updates. See above error for more details.\n"
            )
        return None
    ...
    if parse_version(latestVersion) > parse_version(currentVersion):
        ...
        if silentCheck == False:
            _extracted_from_check_for_update_98(isBeta, currentVersion, latestVersion)
        elif silentCheck == True:
...
# TODO Rename this here and in `check_for_update`
def _extracted_from_check_for_update_98(isBeta, currentVersion, latestVersion):
    print(
        "----------------------------- UPDATE AVAILABLE -------------------------------------------"
    )
    if isBeta == True:
...
def parse_meme(message):
    ...
    return {"meme_text": meme_text, "image_prompt": match.group(4)}


# Sends the user message to the chat bot and returns the chat bot's response
def send_and_receive_message(
    text_model, userMessage, conversationTemp, temperature=0.5
):
    ...
    chatResponse = openai.ChatCompletion.create(
        model=text_model, messages=conversationTemp, temperature=temperature
    )
...
def create_meme(
    ...
    band_height = (
        textbbox_val[3] - textbbox_val[1] + int(font_size * 0.1) + 2 * buffer_size
    )
...
def image_generation_request(apiKeys, image_prompt, platform, stability_api=None):
    if platform == "openai":
        openai_response = openai.Image.create(
            prompt=image_prompt, n=1, size="512x512", response_format="b64_json"
        )
...
def generate(
    ...
    )  # If set to False, will ignore the settings.ini file
    if use_config:
        ...
        image_special_instructions = settings.get(
    ...
    systemPrompt = construct_system_prompt(
    ...
    # Check for updates
    if not noUserInput and release_channel.lower() in ["all", "stable"]:
    ...
    # If any arguments are being used (or set to true for store_true arguments), skip the user input and use the arguments or defaults
        ...
        if not args.userprompt:
            ...
            userEnteredPrompt = input(" >  ") or "anything"
        else:
    ...
    def single_meme_generation_loop():
        ...
        chatResponse = send_and_receive_message(
            text_model, userEnteredPrompt, conversation, temperature
        )
        ...
        virtual_image_file = image_generation_request(
            apiKeys, image_prompt, image_platform, stability_api
        )
    ...
    try:
        ...
        if "The model" in str(irx) and "does not exist" in str(irx):
            ...
            if str(irx) == "The model `gpt-4` does not exist":
                print(
                    "  (!) Note: This error actually means you do not have access to the GPT-4 model yet."
                )
                print(
                    "  (!)       - You can see more about the current GPT-4 requirements here: https://help.openai.com/en/articles/7102672-how-can-i-access-gpt-4"
                )
                print(
                    "  (!)       - Also ensure your country is supported: https://platform.openai.com/docs/supported-countries"
                )
                print(
                    "  (!)       - You can try the 'gpt-3.5-turbo' model instead. See more here: https://platform.openai.com/docs/models/overview)"
                )
            else:
                print(
                    "   > Either the model name is incorrect, or you do not have access to it."
                )
                print(
                    "   > See this page to see the model names to use in the API: https://platform.openai.com/docs/models/overview"
                )
        if not noUserInput:
            input("\nPress Enter to exit...")
        sys.exit()
